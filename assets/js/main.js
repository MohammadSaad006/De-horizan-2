...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (preloaderCanvas) {
        ...
        function animatePreloaderParticles() {
            ...
            preloaderParticles.forEach(p => {
                ...
                pCtx.fillStyle = `rgba(141, 198, 255, ${p.opacity})`;
                pCtx.fill();
            });
            ...
            for (let a = 0; a < preloaderParticles.length; a++) {
                for (let b = a + 1; b < preloaderParticles.length; b++) {
                    ...
                    if (dist < 120) {
                        pCtx.beginPath();
                        pCtx.strokeStyle = `rgba(141, 198, 255, ${0.08 * (1 - dist / 120)})`;
                        pCtx.lineWidth = 0.5;
    ...
    if (dustCanvas) {
        ...
        const goldColors = [
            'rgba(141, 198, 255,',   // Classic gold
            'rgba(141, 198, 255,',  // Light gold
            'rgba(228, 241, 254,',  // Pale gold shimmer
            'rgba(52, 73, 94,',   // Deep gold
            'rgba(34, 49, 63,',   // Warm gold
        ];
    ...
    revealStyle.textContent = `
        ...
        .card-tilt-active:hover { box-shadow: 0 25px 60px rgba(141, 198, 255,0.2), 0 5px 15px rgba(0,0,0,0.1) !important; }

        /* --- Floating Glow Pulse for Cards --- */
        @keyframes glowPulse { 0%, 100% { box-shadow: 0 5px 20px rgba(141, 198, 255,0.08); } 50% { box-shadow: 0 8px 35px rgba(141, 198, 255,0.18); } }
        ...
        .testimonial-card:hover { transform: translateY(-8px) scale(1.02) !important; border-color: rgba(141, 198, 255,0.5) !important; }
        .testimonial-card:hover .testimonial-author img { transform: scale(1.1); box-shadow: 0 0 20px rgba(141, 198, 255,0.4); }
        ...
        @keyframes badgeBreath { 0%, 100% { transform: scale(1); box-shadow: 0 10px 30px rgba(141, 198, 255,0.2); } 50% { transform: scale(1.04); box-shadow: 0 15px 45px rgba(141, 198, 255,0.35); } }
        ...
        class GoldParticle {
            ...
            draw() {
                ...
                if (this.size > 1.5) {
                    ...
                    ctx.lineTo(-this.size * 0.6, 0);
                    ctx.closePath();
                    ctx.fillStyle = `rgba(141, 198, 255, ${this.opacity})`;
                    ctx.fill();
                } else {
                    ctx.beginPath();
    ...
    document.addEventListener('click', (e) => {
        ...
        ripple.style.cssText = `
            ...
            width: 0;
            height: 0;
            border: 2px solid rgba(141, 198, 255, 0.5);
            border-radius: 50%;
            pointer-events: none;
            z-index: 99999;
